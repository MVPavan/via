...
SDKS TO COVER: GitHub Copilot SDK (all languages); Cursor SDK (Python, TypeScript, SDK Bridge); Google Antigravity SDK (google-antigravity Python); any official Gemini CLI SDK or programmatic package (check @google/gemini-cli-core or similar); Meta Muse Code (any SDK or muse serve client)
For Copilot check whether personal Copilot plans (Free/Pro/Pro+) work with the SDK and billing via premium requests/AI credits. For Cursor check the AUP 'automated or non-human means' clause vs SDK docs. For Antigravity check whether the SDK can use a Google AI Pro/Ultra subscription or only Gemini API key / Vertex, and the Antigravity terms on third-party OAuth.
