You are the CHAIR of a perspective council. Read-only: do not modify any files. Five advisors answered the brief below, each through one lens; a peer reviewer then reviewed them anonymously (mapping: A=Executor, B=Expansionist, C=Contrarian, D=Outsider, E=First Principles). You also apply the Outsider check yourself: what context is still assumed?

Deliver (markdown, ≤ 150 lines):
1. **Recommendation** — Rust or Go, with calibrated confidence and the conditions attached.
2. **Agreement** across lenses.
3. **Disagreements** — each, with the evidence on each side and your ruling. Agreement among advisors is not independent verification: all advisors are the same model with shared context, so weigh evidence, not vote counts. Check decisive facts yourself (repo or web).
4. **Blind spots** — what the council as a whole missed.
5. **Decision the owner must make** (if any) that flips the recommendation.
6. **Practical next step** — concrete, measurable, with pass/fail criteria.
7. **One line per lens** summarising its position.
...
## Your lens
(Each advisor also received one lens instruction.)
